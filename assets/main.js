(function () {
  ...
  function tryInject() {
    ...
    const observer = new MutationObserver(() => {
      ...
      stickyBtn.innerHTML = orig.innerHTML;
    });
    observer.observe(orig, { attributes: true, attributeFilter: ['aria-label'] });
    ...
    extraStyle.textContent = `
      button.button.button--large.zrx-sticky-atc-wishlist-button span {
        display: flex;
        flex-direction: row;
        align-items: center;
      }
      ...
      .zrx-sticky-atc-product-actions .zrx-sticky-atc-wishlist-button[aria-label="Wishlist"] {
        background-color: rgb(179 137 51 / 0%) !important;
        color: #cab243 !important;
        border: 2px solid #cab243 !important;
        border-radius: 8px;
      ...
      .zrx-sticky-atc-product-actions .zrx-sticky-atc-wishlist-button[aria-label="Wishlist"] svg {
        color: #cab243 !important;
      }

      .zrx-sticky-atc-product-actions .zrx-sticky-atc-wishlist-button[aria-label="REMOVE FROM WISHLIST"] {
        background: #cab243 !important;
        border-radius: 8px !important;
      }

      .zrx-sticky-atc-product-actions .zrx-sticky-atc-wishlist-button[aria-label="REMOVE FROM WISHLIST"] span {
...
class DetailsDisclosure extends HTMLElement {
  constructor() {
    ...
    this.svg = this.querySelector(".pro-dropdown");
  ...
  connectedCallback() {
    ...
    this.delegatedToggleFiltersCallback = theme.addDelegateEventListener(this.section, 'click', '[data-toggle-filters]', (evt) => {
      ...
      if (window.matchMedia("(max-width: 768px)").matches) {
        ...
        if(bodyTag){
           bodyTag.classList.toggle('filter-open');
        }
      }
  ...
  ajaxLoadUrl(url, noPushState) {
    ...
    fetch(fetchUrl, {
      ...
      .then((response) => {
        ...
        // init js
        this.initFiltersEtc();
        if (window.__kisahInfiniteScrollManager && typeof window.__kisahInfiniteScrollManager.boot === 'function') {
       ...
       // manic comment const scrollToY = theme.getOffsetTopFromDoc(this.section.querySelector('[data-ajax-scroll-to]')) - document.querySelector('.section-header').clientHeight;
        const scrollToY =  document.querySelector('.section-header').clientHeight;
        window.scrollTo({
          top: scrollToY,
          behavior: 'smooth'
        });
...
const LinkDropdown = class extends HTMLElement {
  ...
  toggle(evt, isDismiss) {
    ...
    //manic code start
    let btn = this.button.parentElement.parentElement;
    let bodyElement = document.querySelector('body');
    if(bodyElement){
      bodyElement.classList.toggle('sortfilter');
    }
    btn.classList.toggle('special');
    // manic code end
  ...
  connectedCallback() {
    ...
    document.querySelector('.section-header').insertAdjacentElement('afterend', this.mobileDrawer);
      ...
      evt.preventDefault();
        ...
        const navChildren = this.mobileDrawer.querySelectorAll('.navigation__tier-1 > .navigation__item--open');

       if (doOpen) {
         delEl.parentElement.classList.add('navigation__item--open');
          // delEl.nextElementSibling.style.height= `${delEl.nextElementSibling.firstElementChild}px`;
           delEl.nextElementSibling.style.height= delEl.nextElementSibling.firstElementChild.style.height='100%';
      ...
      // scroll container
       this.mobileDrawer.closest('.mobile-navigation-drawer').scrollTo({ top: 0, left: 0, behavior: 'instant' });
     // 'smooth' not working in iOS 15
    });
...
const ProductForm = class extends HTMLElement {
  ...
  async handleSubmit(evt) {
    ...
    // --- Child Product (Also Buy) Logic ---
    const childVariantInput = document.getElementById('js-child-variant-id');
    const childProductId = childVariantInput ? childVariantInput.value : null;

    if (childProductId) {
      const childFormData = new FormData();
      childFormData.append('id', childProductId);
      childFormData.append('quantity', formData.get('quantity') || 1);
      ...
      try {
        ...
        console.error('Child product cart add failed:', e);
      }
    }
    // --- End Child Product Logic ---
